            ...
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                ...
                <div class="pan-header-right d-flex align-items-center gap-3">
                    ...
                    <div class="pan-charge-card">
                        <span class="pan-charge-label">Service Charge</span>
                        <span class="pan-charge-amount">₹{{ number_format($otherServiceCharge, 2) }}</span>
                    </div>
            ...
            <form id="aadhaarForm" enctype="multipart/form-data">
                ...
                <div class="row g-4">
                    @foreach($fields as $field)
                        <div class="{{ ($field['type'] ?? 'text') === 'textarea' ? 'col-12' : 'col-xl-6 col-lg-6 col-md-6' }}">
                            <div class="aadhaar-field-card">
                                ...
                                @if(($field['type'] ?? '') === 'textarea')
                                    ...
                                    >{{ old($field['name'], $data['form_data'][$field['name']] ?? '') }}</textarea>
                                ...
                                @elseif(($field['type'] ?? '') === 'file')
                                    ...
                                    @if(!empty($files[$field['name']]))
                                        <div class="uploaded-file-box">
                                            <a href="{{ file_url($files[$field['name']]) }}" target="_blank" class="aadhaar-file-btn">
                                ...
                                @elseif(($field['type'] ?? '') === 'select')
                                    ...
                                    >
                                        ...
                                        @foreach($field['options'] ?? [] as $option)
                                            <option
                                                value="{{ $option }}"
                                                {{ old($field['name'], $data['form_data'][$field['name']] ?? '') == $option ? 'selected' : '' }}
                                            >
                                                {{ $option }}
                                            </option>
                                        @endforeach
                                ...
                                @elseif(($field['type'] ?? '') === 'date')
                                    <input
                                        ...
                                        name="{{ $field['name'] }}"
                                        class="form-control aadhaar-input"
                                        value="{{ old($field['name'], $data['form_data'][$field['name']] ?? '') }}"
                                    >
                ...
                <div class="aadhaar-field-card mt-4">
                    <label class="aadhaar-label">Remarks</label>
                    <textarea
                        name="remarks"
                        rows="5"
                        class="form-control aadhaar-input"
                    >{{ old('remarks', $data['form_data']['remarks'] ?? '') }}</textarea>
                </div>

                {{-- ACTION --}}
                <div class="aadhaar-action-area">
                    <button type="submit" class="aadhaar-submit-btn">
...
$(document).ready(function () {
    ...
    let guidelineOffcanvasEl = document.getElementById('serviceGuidelineOffcanvas');

    if (guidelineOffcanvasEl) {
        guidelineOffcanvasEl.addEventListener('show.bs.offcanvas', function () {
            let iframe = document.getElementById('guidelinePdfFrame');
    ...
    $(document).on('change', '.state-dropdown', function () {
        ...
        $.get(
            "{{ route('retailer.other-service.districts.by-state', '') }}/" + stateId,
            function (response) {
